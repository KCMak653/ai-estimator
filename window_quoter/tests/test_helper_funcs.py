...
class TestHelperFuncs(unittest.TestCase):
    def test_calculate_sf(self):
        ...
        self.assertEqual(calculate_sf(0, 36), 0)
        self.assertEqual(calculate_sf(24, 0), 0)
        self.assertEqual(calculate_sf(0, 0), 0)
        
        # Test with negative dimensions
        self.assertEqual(calculate_sf(-24, 36), 0)
        self.assertEqual(calculate_sf(24, -36), 0)
        self.assertEqual(calculate_sf(-24, -36), 0)
    ...
    def test_calculate_lf(self):
        ...
        # Test with zero dimensions
        self.assertEqual(calculate_lf(0, 36), 6.0)  # 2 * (0 + 36) / 12 = 6.0
        self.assertEqual(calculate_lf(24, 0), 4.0)  # 2 * (24 + 0) / 12 = 4.0
        self.assertEqual(calculate_lf(0, 0), 0.0)  # 2 * (0 + 0) / 12 = 0.0
        
        # Test with negative dimensions
        self.assertEqual(calculate_lf(-24, 36), 0)
        self.assertEqual(calculate_lf(24, -36), 0)
        self.assertEqual(calculate_lf(-24, -36), 0)
    ...
    def test_calculate_price_from_brackets(self):
        ...
        brackets = [(10, 100, 5), (20, 150, 10), (30, 200, 15)]
        self.assertEqual(calculate_price_from_brackets(5, brackets), 100)
        self.assertEqual(calculate_price_from_brackets(10, brackets), 100)
        self.assertEqual(calculate_price_from_brackets(15, brackets), 125)  # 100 + (15-10)*5
        self.assertEqual(calculate_price_from_brackets(20, brackets), 150)
    ...
    def test_get_base_price(self):
        ...
        # Create a mock pricing config
        pricing_config = {
            "casement": {
                "white": [(10, 100, 5), (20, 150, 10)],
                "paint": [(10, 120, 6), (20, 180, 12)]
            }
        }
        
        # Test with valid inputs
        self.assertEqual(get_base_price("casement", "white", pricing_config, 5), 100)
        self.assertEqual(get_base_price("casement", "white", pricing_config, 15), 125)  # 100 + (15-10)*5
        self.assertEqual(get_base_price("casement", "paint", pricing_config, 5), 120)
        self.assertEqual(get_base_price("casement", "paint", pricing_config, 15), 150)  # 120 + (15-10)*6
